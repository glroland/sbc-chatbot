...
vdb_client.load_collection(collection_name=VDB_COLLECTION_PHRASES)
...
if prompt := st.chat_input("Type your question here..."):
    ...
    search_res = vdb_client.search(
        collection_name=VDB_COLLECTION_PHRASES,
        data=[
            encode_text(model, prompt)
        ],  # Use the `emb_text` function to convert the question to an embedding vector
        limit=3,  # Return top 3 results
        search_params={"file": option}, 
